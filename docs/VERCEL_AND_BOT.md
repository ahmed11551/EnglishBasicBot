...
**Если пишет «Подписка временно недоступна» или «подписка пока не настроена»:** значит хранилище KV не подключено или недоступно. Сделайте: **Vercel → проект → Storage** → создайте базу **KV** (если ещё нет) → **Connect to Project** → выберите этот проект. В переменные окружения автоматически добавятся `KV_REST_API_URL` и `KV_REST_API_TOKEN`. Выполните **Redeploy**. После этого подписка должна заработать.
...
3. Введите имя бота (например: **МГИМО BASIC ENGLISH**).
...
2. Добавьте переменные для **бота BASIC**:
   - **Name:** `BOT_TOKEN`  
